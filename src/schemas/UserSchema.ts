...
export const UserManyToManyRelationsValues = ['permissions', 'authorizedClients', 'authorizedCompanies'] as const
export const UserRelationsValues = [...UserManyToManyRelationsValues, 'company', 'roleEntity'] as const
...
export class UserSchema {
  ...
  static password = Joi.string()
    .pattern(/^(?=.*[a-zA-Z])(?=.*[0-9])[a-zA-Z\d`~!@#$%^&*_|+\-=?;:'",.\\]{8,40}$/)
  ...
  static authorizedClients = Joi.array().unique().items(Joi.number().optional())
